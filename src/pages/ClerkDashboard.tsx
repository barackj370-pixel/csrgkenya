...
export default function ClerkDashboard() {
  ...
  const handleGroupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editGroupId) {
        await supabase.from('Group').update(payload).eq('id', editGroupId);
      } else {
        await supabase.from('Group').insert([payload]);
      }
  ...
  const handleIssueSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editIssueId) {
        await supabase.from('Issue').update(payload).eq('id', editIssueId);
      } else {
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-stone-900">Clerk Portal</h1>
