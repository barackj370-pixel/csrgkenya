    ...
    async function fetchData() {
      try {
        const [wardsRes, issuesRes, discussionsRes] = await Promise.all([
          supabase.from('Ward').select('*, issues(id), discussions(id), groups(id)'),
          supabase.from('Issue').select('*'),
          supabase.from('Discussion').select('*, ward:Ward(*)')
        ]);
        
        const wardsData = wardsRes.data?.map(w => ({
          ...w,
          _count: {
            issues: w.issues?.length || 0,
            discussions: w.discussions?.length || 0,
            groups: w.groups?.length || 0
          }
        })) || [];
        ...
        const past = discussionsRes.data 
